...
#2 initialisation of the camera
...
#3 Capturing the initial frame for creation of background
while (True):
    cv2.waitKey(1000)
...
#4 Start capturing the frames for actual magic!!
while (True):
    ret, frame = cap.read()
...
#5 Kernel to be used for dilation
...
#6 The mixing of frames in a combination to achieve the required frame
    b = frame[:, :, 0]
    g = frame[:, :, 1]
    r = frame[:, :, 2]
